  ...
  async function incrementCategoryCount(category) {
    try {
      const res = await fetch('/api/incrementCategoryCount', {
        ...
        body: JSON.stringify({ category: category, origin: 'DSA' }),
      });
  ...
  async function getConversationById(conversationId) {
    try {
      const res = await fetch('/api/getConversation', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ conversationId, origin: 'DSA' }),
      });
  ...
  useEffect(() => {
    ...
    const sendHeartbeat = async () => {
      ...
      await fetch('/api/heartbeat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ sessionId: session?.user.id, origin: 'DSA' }),
      });
    };
    sendHeartbeat();
        ...
        >
          ...
          {settingsFlag && (
            <ul className={styles.options}>
              ...
              <li>
                <Link href={'/DSA/about'}>About</Link>
              </li>
              <li>
